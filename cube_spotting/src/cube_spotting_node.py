...
def getDistance(pixelHeight):
  focalLength_mm = 3.67
  imageHeight = 480
  realHeight = 53
  diam = 16/2.88
  sensorHeight = 3 * (diam**2 / 25)**0.5
...
def mask2box(mask,colour,canvas,minArea):
  ...
  for i, c in enumerate(minRect):
    ...
    tempCube.height = max(size) # bad code probably change |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    tempCube.distance = getDistance(tempCube.height)
...
class cubeSpotter:
  ...
  def __init__(self):
    ...
    # Yellow - H=30
    self.hsvYellowLow=(22.0000, 118.0000, 61.0000)
    self.hsvYellowHigh=(40.0000, 255.0000, 255.0000)

    # Blue
    self.hsvBlueLow=(80.0000, 70.0000, 82.0000)
    self.hsvBlueHigh=(120,255,255)

    # Red - wraps around 0, but the red blocks are mostly in the 0-10 range
    self.hsvRedLow1=(0.0000, 140.0000, 70.0000)
    self.hsvRedHigh1=(20,255,255)
    self.hsvRedLow2=(160.0000, 100.0000, 70.0000)
  ...
  def process(self):
    ...
    erodedMaskRed=erode(maskRed, 5)
    erodedMaskBlue=erode(maskBlue, 5)
    erodedMaskYellow=erode(maskYellow, 5)    
    ...
    # UNCOMMENT THIS FOR OTHER COLOURS |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    for c in range(len(cubeListBlue)):
      ...
      tempCube.height = cubeListBlue[c].height
      tempCube.distance = cubeListBlue[c].distance
      returnCubeArray.cubes.append(tempCube)
